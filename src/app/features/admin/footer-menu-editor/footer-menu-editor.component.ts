...
@Component({
  ...
  templateUrl: './footer-menu-editor.component.html',
  styleUrls: ['./footer-menu-editor.component.scss']
})
export class FooterMenuEditorComponent implements OnInit {
    ...
    this.menuService.getMenu(MenuType.Footer).subscribe({
      next: (menu: MenuDto | null) => {
        if (menu) {
          ...
          this.menu.set({ type: MenuType.Footer, menuItems: [] });
        }
  ...
  private saveMenu(menu: MenuDto): void {
    ...
    console.log(menu.id)
    if (menu.id) {
      this.menuService.update(menu.id, request).subscribe({
        next: () => {
          this.menu.set(menu);
        },
        error: (err: any) => {
